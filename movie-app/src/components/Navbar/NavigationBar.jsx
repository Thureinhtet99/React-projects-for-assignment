...
import { apiKey, baseApi } from "../../api/api";
import { useDispatch } from "react-redux";
import { fetchMovies } from "../../redux/action/movies";

const NavigationBar = () => {
  // createRef
  const movieSearchKeyRef = createRef();
  // useDispatch
  const dispatchSearchedMovie = useDispatch();

  // Handle movie search
  const handleMovieSearchForm = async (event) => {
    event.preventDefault();
    if (movieSearchKeyRef.current.value) {
      const response = await baseApi.get(
        `search/movie?query=${movieSearchKeyRef.current.value}&api_key=${apiKey}`
      );
      dispatchSearchedMovie(fetchMovies(response.data.results));
  ...
  return (
    <>
      <Navbar fluid className="sticky top-0 bg-gray-200">
        <Navbar.Brand href="/">
          <img
        ...
        <div className="flex md:order-2">
          <form onSubmit={handleMovieSearchForm}>
            <div className="flex justify-center items-center">
              <TextInput
                className="mx-1"
                sizing="sm"
                placeholder="Search here...."
                ref={movieSearchKeyRef}
              />
              <Button type="submit" className="text-black border-black">
